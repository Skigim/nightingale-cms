...
function parseAvsAccountBlock(block, knownAccountTypes) {
    ...
    const sortedAccountTypes = [...knownAccountTypes].sort((a, b) => b.length - a.length);
    ...
    const ownerList = owners.split(';').map(o => o.trim()).join(', ');
    const bankLine = lines[1] || '';
    const bankNameMatch = bankLine.match(/([^\n]+) - \(/);
    const accountNumberMatch = bankLine.match(/ - \((\d+)\)/);
    
    const balanceLine = lines.find(l => l.toLowerCase().includes('balance as of'));
    const balanceMatch = balanceLine ? balanceLine.match(/Balance as of .* - (.*)/) : null;
    ...
    return {
        ...
        value: balanceMatch ? parseFloat(balanceMatch[1].replace(/[^0-9.]/g, '')) || 0 : 0,
        verificationStatus: "Verified", // Data from AVS is considered verified
        source: `AVS as of ${new Date().toLocaleDateString("en-US")}`
    };
}
...
function parseAvsData(rawInput, knownAccountTypes) {
    ...
    const accountBlocks = rawInput.split(/Account Owner: /).filter(block => block.trim() !== '');
    if (accountBlocks.length === 0) {
        return [];
    }
    // Parse each block and filter out any that are invalid
    const parsedAccounts = accountBlocks.map(block => parseAvsAccountBlock(block, knownAccountTypes)).filter(Boolean);
