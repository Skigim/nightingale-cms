...
window.parseTransactions = function(text, confidence = 0) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^(Date|Transaction|Description|Amount|Balance)/i) || 
        line.length < 10 || 
        !line.match(/\d/)) {
      continue;
    }
    ...
    try {
      ...
      if (totalAmount > 0) {
        ...
        if (amountStr.length > 4 && !amountStr.includes('.') && totalAmount % 1 === 0) {
          ...
          if (possibleDecimal < 10000 && possibleDecimal > 0.01) {
            parsingErrors.push(`Missing decimal? Amount: $${totalAmount} might be $${possibleDecimal.toFixed(2)}`);
          }
        }
      }

      // Balance validation (parsing error - calculation issue)
      const previousBalance = allTransactions.length > 0 ? allTransactions[allTransactions.length - 1].balance : null;
      ...
      if (confidence < 60) {
        ocrUncertainty.push(`Low source OCR confidence: ${confidence.toFixed(1)}%`);
      }
...
window.processTransactionsWithSummary = function(text, ocrConfidence = 0) {
  ...
  console.log(`  🔍 OCR uncertainty (text recognition): ${totalOcrUncertainty}`);
  console.log(`  📈 Warning rate: ${((totalWarnings / totalTransactions) * 100).toFixed(1)}%`);
