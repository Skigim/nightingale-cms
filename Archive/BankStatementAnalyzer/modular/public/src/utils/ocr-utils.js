...
window.stage1_PrepareImage = async function(canvas) {
  ...
  try {
    ...
    const { contrast, brightness, gaussianBlur } = window.OCRConfig.jimp.preprocessing;
    ...
    return {
      ...
      message: '✅ Stage 1 Complete: Image preprocessed with enhanced contrast and noise reduction'
    };
...
window.stage3_RefineData = async function(ocrResult) {
  ...
  try {
    ...
    const bankingTerms = [
      'BALANCE', 'DEPOSIT', 'WITHDRAWAL', 'TRANSFER', 'PAYMENT', 'CHECK', 'FEE',
      'INTEREST', 'DIVIDEND', 'REFUND', 'PURCHASE', 'DEBIT', 'CREDIT', 'ACH',
      'WIRE', 'ATM', 'POS', 'OVERDRAFT', 'NSF', 'RETURNED', 'CLEARED',
      'PENDING', 'AUTHORIZED', 'DECLINED', 'APPROVED', 'TRANSACTION'
    ];
    ...
    const enhancementConfidence = bestFuseResults.length > 0 
      ? 1 - (bestFuseResults.reduce((sum, r) => sum + r.score, 0) / bestFuseResults.length)
      : 0.5;
